...
% if encoding='gzip', the unu command is used to zip the nrrd file.
% That's why the unu command needs to be available on the system.
%
...
if ( isfield( nrrdStructure, 'encoding' ) )
  fprintf( fid, 'encoding: %s\n', nrrdStructure.encoding );
else 
...
% if encoding is "gzip", the raw data needs to be zipped:
if (strcmp(nrrdStructure.encoding,'gzip'))
    fprintf('Zipping the raw data ... ');
    unuCommand = sprintf('unu save -f nrrd -e gzip -en little -i %s -o %s', nhdrFileName, nhdrFileName);
    system( unuCommand );
    %  now delete the remaining .raw file
    [pathstr,matFileName,ext] = fileparts( nhdrFileName );
    rawname = strcat(pathstr,'/',matFileName,'.raw');
    delete(rawname);
    fprintf('Done.\n');
end
