...
% fid1 = fopen(['log_', option.name,'loadData1'],'w'); %Changed from:  fid1 = fopen(['./log/log_', option.name,'loadData1'],'w');
...
for s=1:length(SITES)
    ...
    if(isempty(dwipaths))
        ...
        if(t(2)<2)
            ...
            return;
        elseif(t(2)==3) % Bruno 25-06 change from 2 to 3 so it can read the WMH files
            ...
            for i=1:t(1)
                ...
                if exist(dwipaths{i,1}, 'file') &&  exist(dwipaths{i,2}, 'file')
                    ...
                    %read WMH Bruno - 25-06
                    idxSlash = strfind(dwipaths{i,3},'/'); wmhpath=dwipaths{i,3};
                    wmhtype = wmhpath(idxSlash(end)+1:end);
                    idxDot=strfind(wmhtype,'.');
                    SITES{s}.InputImages{i}.WMHName=wmhtype(1:idxDot(1)-1);
                    SITES{s}.InputImages{i}.WMHFullPath=maskpath;
                    SITES{s}.InputImages{i}.WMHDirectory=wmhpath(1:idxSlash(end));
                    SITES{s}.InputImages{i}.WMHImageType=wmhtype(idxDot(1)+1:end);
                    %read WMH Bruno - 25-06
                    ...
                    SITES{s}.InputImages{i}.WMHSuffix = '';  %added Bruno 25-06
                    ...
                    SITES{s}.InputImages{i}.DWISuffix = '';
                    SITES{s}.InputImages{i}.MaskSuffix = ''; 
                    SITES{s}.InputImages{i}.WMHSuffix = ''; %added 25-06
                    SITES{s}.InputImages{i}.harmonized = 0;
...
% % read the WMH Bruno 25-06
fid = fopen([filefullpath, '_wmh.txt']);
tline =  fgetl(fid);
i = 1;
while ~feof(fid)  && ~isempty(tline) && ~strcmpi(tline, 'nan')
    paths{i,3} = tline;
