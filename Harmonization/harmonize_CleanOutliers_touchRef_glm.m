...
function [HarmonizedDWIPaths, SITES]  = harmonize_CleanOutliers_touchRef_glm(SITES, templatesPath, coption)
...
for do_harmonize=0:1%0:1 % when = 0 just produce the rished, when = 1 also harmonize
% if(s == 1 && do_harmonize == 1)
%     % not going to harmonize the ref (not true anymore given we harmonize
%     % some features of the ref as well. The covariates are subtracted also from the ref)
%     continue
% end
...
for i=1:SITES{s}.ImageNum
    ...
    if ~exist([data.Image_Harmonized_dir, coption.harmonizedName, data.ImageName,...
        ...
        for L=0:2:SHorder
            ...
            mov=[templatesPath 'Scale_L' num2str(L) '_S' num2str(s) '.nii.gz'];
            
            fix=[ rishPath,    '_L'  num2str(L) '.nii.gz'];
            
            mov_warped=[data.Image_Harmonized_dir,     'rish/Scale_L' num2str(L) '_S' num2str(s) '_' data.ImageName  '.nii.gz'];
            ...
            mov2 = [templatesPath 'L_' num2str(L) '_RISHGLM_beta.nii.gz'];
            beta = load_untouch_nii(mov2);
            all_betas = [];
            for betai=1:size(beta.img,4)
                beta1v = beta;
                beta1v.img = beta.img(:,:,:,betai);
                beta1v.hdr.dime.dim([1 5]) = [3 1];
                mov2v = [templatesPath 'L_' num2str(L) '_RISHGLM_beta_' num2str(betai) '.nii.gz'];
                save_untouch_nii(beta1v,mov2v);
                mov_warped2 = [data.Image_Harmonized_dir,     'rish/Scale_L' num2str(L) '_beta_' num2str(betai) '_' data.ImageName  '.nii.gz'];
                [status,out] =system(['antsApplyTransforms -d 3 -i '  mov2v ' -o ' mov_warped2 ' -r  '  fix      '   -t '  t_img1  '    -t '  t_img2 '.mat' ]);
                beta_warped = load_untouch_nii(mov_warped2);
                all_betas = cat(4,all_betas,beta_warped.img);
            end
...
%             Cs2_warped.img = sqrt(Cs2_warped.img);
            [sx,sy,sz,st] = size(all_betas);
            all_betas = reshape(all_betas,sx*sy*sz,st);
            
            for SH_levels=[IMG.shs_same_level(L/2+1,1):IMG.shs_same_level(L/2+1,2)]
                new_Cs = (Cs2_warped.img(:)').*IMG.Cs(SH_levels,:);
                if(~isempty(coption.covariates_2_correct))
                    new_Cs = new_Cs - all_betas(:,coption.covariates_2_correct)'*coption.covariates{s}(i,coption.covariates_2_correct);
                end
                mapped_Cs = [mapped_Cs; new_Cs];
            end
        end
